...
def full_multiclass_report(model: keras.Model,
    ...
    if history is not None:
        val_acc_list = [s for s in history.history.keys() if
                        'acc' in s and 'val' in s]
        comment += '\n'
        comment += 'Max accuracy : {}'.format(max(history.history[val_acc_list[0]]))
...
def slack_report(x_train: np.ndarray,
    ...
    upload_to_slack('/tmp/loss.png', f'{name}-{str(model_params)}', token)
    upload_to_slack('/tmp/acc.png', f'{name}-{str(model_params)}', token)
    # x_mean = np.array([x_train[:, :, :, 0].mean(),
    #                    x_train[:, :, :, 1].mean(),
    #                    x_train[:, :, :, 2].mean()])
    # x_std = np.array([x_train[:, :, :, 0].std(),
