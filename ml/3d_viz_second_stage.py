...
def make_preds():
    ...
    for h, blob in enumerate(blobs):
        ...
        for i in range(0, len(arr), 32):
            ...
            layer_90 = np.zeros(shape=(int(math.ceil(len(arr[0]) / 32) * math.ceil(len(arr[0][0]) / 32))))
            layer_idx = 0
            for j in range(0, len(arr[0]), 32):
                for k in range(0, len(arr[0][0]), 32):
                    ...
                    layer_idx += 1
            layer = np.reshape(layer, (int(math.ceil(len(arr[0]) / 32)), int(math.ceil(len(arr[0][0]) / 32))))
            layer_90 = np.reshape(layer_90, (int(math.ceil(len(arr[0]) / 32)), int(math.ceil(len(arr[0][0]) / 32))))
            preds.append(layer)
