...
for oldname in os.listdir(path):
    ...
    if oldname in IDs:
        ...
        try:
            os.rename(os.path.join(path, oldname), os.path.join(tmpPath, (IDs[oldname_wo_number] + "_1")))
        except:
            print('File ' + oldname + ' could not be renamed to ' + IDs[oldname_wo_number] + "_1" + '!')
    elif oldname.replace("_2", "") in IDs:
        oldname_wo_number = oldname.replace("_2", "")
        try:
            os.rename(os.path.join(path, oldname), os.path.join(tmpPath, (IDs[oldname_wo_number] + "_2")))
        except:
            print('File ' + oldname + ' could not be renamed to ' + IDs[oldname_wo_number] + "_2" + '!')

    elif oldname.replace("_3", "") in IDs:
        oldname_wo_number = oldname.replace("_3", "")
        try:
            os.rename(os.path.join(path, oldname), os.path.join(tmpPath, (IDs[oldname_wo_number] + "_3")))
        except:
            print('File ' + oldname + ' could not be renamed to ' + IDs[oldname_wo_number] + "_3" + '!')
