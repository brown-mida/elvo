...
def save_npy_to_cloud(arr: np.ndarray, id: str, whence: str):
    """Uploads MIP .npy files to gs://elvos/overlap_mip_data/from_numpy/
        <patient id>.npy
    """
    try:
        print(f'gs://elvos/overlap_mip_data/from_{whence}/{id}.npy')
        np.save(file_io.FileIO(f'gs://elvos/overlap_mip_data/from_'
                               f'{whence}/{id}.npy', 'w'), arr)
    except Exception as e:
        logging.error(f'for patient ID: {id} {e}')
...
if __name__ == '__main__':
    ...
    for in_blob in bucket.list_blobs(prefix='preprocess_luke/training/'):
        ...
        save_npy_to_cloud(axial, in_blob.name[25:41], 'luke')
    ...
    for in_blob in bucket.list_blobs(prefix='preprocess_luke/validation/'):
        ...
        save_npy_to_cloud(axial, in_blob.name[27:43], 'luke')
