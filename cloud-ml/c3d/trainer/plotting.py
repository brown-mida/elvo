...
def upload_gcs_plots(x_train: np.ndarray,
    ...
    """
    if plot_dir is None:
        plot_dir = pathlib.Path('/tmp/{}'.format(job_name))
    ...
    for filename in os.listdir(str(plot_dir)):
        blob = bucket.blob(
            'plots/{}-{}/{}'.format(job_name, created_at, filename))
...
def full_multiclass_report(model: keras.models.Model,
    ...
    try:
        ...
        comment += 'Log loss: {}\n'.format(log_loss)
        comment += 'F-1: {}\n'.format(sklearn.metrics.f1_score(y_true, y_pred))
    ...
    for i in (0, 1):
        for j in (0, 1):
            ...
            plot_misclassification(x_filtered,
                                   ...
                                   ids=ids_filtered,
                                   chunk=chunk)
            plt.savefig(str(plot_name_dict[(i, j)]))
